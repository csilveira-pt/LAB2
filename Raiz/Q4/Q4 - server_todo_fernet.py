...
class TodoList:
    ...
    def save_cipher(self, key):
        cipher = Fernet(key)
        with open('todo_list.fernet', 'wb') as file:    #Correção de w para wb uma vez que key.fernet é binário
...
try:
    with open('todo_list.fernet', 'rb') as file:  #'rb' leitura como binário
        todo_list = TodoList()
        for line in file:
            cipher = Fernet(key)
            c_item = line.strip()
            ...
            title, description, completed = plain_item.split(",") #Tem de ter em consideração o estdao da tarefa
            item = TodoItem(title, description)
